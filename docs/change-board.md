...
- **Files changed (actual):** `web/app/operate/calls/page.tsx` (LADDER TIER cell gets T<n> prefix), `web/app/operate/review/[id]/page.tsx` (stub-turn rationale → Seeded sample chip; is_stub badge in outcome strip), `web/lib/labels.ts` (humanizeRationale returns '' for sim-harness; isSimHarnessRationale exported), `tests/e2e/qa7_cb65_leaksweep.py` (3 new CB-94 tests + "Simulated training call" leak pattern).
- **What changed:**
  - **(a) OUTCOME vs LADDER TIER duplication — real cosmetic, not a render bug.** Investigation: the data shows they genuinely share the same text (e.g. "Consultation booked") for bookings because the tier label and outcome label are intentionally aligned — it IS the same concept. The fix: prefix the LADDER TIER cell with a small muted "T<n>" rung badge (e.g. "T2 Consultation booked") so the column is visually distinct and the rung position is scannable at a glance. The OUTCOME column's dot-tag remains unchanged. No data change; client-side render only.
  - **(b) Stage label "Closing" on a discovery turn — OUT OF SCOPE.** Investigation: the stage chip on each turn row renders `t.belief?.stage`, which is a serialized field from `src/core`'s BeliefSnapshot. If the brain stored "Closing" on turn 2, it's stale data from the brain's own classifier — not a client-side derivation we can fix. Touching this would require modifying `src/core/respond.py` or the BeliefSnapshot serialization, which is outside the web-only scope of CB-94 and violates R37. Logged as a `src/core` data quality issue; no change made.
  - **(c) Stub-turn rationale "Simulated training call" — fixed.** `humanizeRationale` previously mapped "sim-harness decision" → "Simulated training call" (CB-81). CB-94 replaces that with: (1) `humanizeRationale` returns `''` for sim-harness turns; (2) `isSimHarnessRationale()` exported from labels.ts; (3) the review page renders a small "Seeded sample" chip instead of a rationale quote for stub turns; (4) a "Seeded sample" badge added to the review page's outcome strip when `ep.is_stub` is true — mirroring the calls-list badge so operators opening a stub review directly see the provenance.
- **Verification:** qa7_cb65_leaksweep.py 17/17 (3 new CB-94 tests + the "Simulated training call" leak pattern added; no prior test broken); qa7_cb60.py 6/6; qa7_cb66.py 15/15; qa9fix_ops.py 11/11 (1 skip); tsc clean. "Simulated training call" and "sim-harness decision" absent from rendered call review text.
- **Constraints checked:** R37 (no src/core touched; (b) explicitly waived as core-data-out-of-scope); no internal indices in observable output; leak sweep stays green.
- **Follow-ups / known gaps:** (b) stage label mis-classification on seeded stubs is a src/core data quality issue — the belief serializer stores the stage at commit time from the brain's internal state machine; a stale "Closing" on turn 2 means the seeded stub was generated by a brain version whose stage classifier ran ahead of the turn content. Needs a replay/re-seed fix in src/core if this affects real calls too (outside this item's scope).
